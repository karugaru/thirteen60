...
調べた限り、6uスペースバーの流通は非常に限定的なので、そのレイアウトを使用する場合はご注意ください。
...
ケースの加工をする場合は、ケース側のネジを止める土台をニッパーなどで切って除去してください。(画像の赤矢印で示した、基板下の部分)
...
スイッチの加工をする場合は、スイッチがネジの土台と干渉しなくなるまで、中央部分(下記画像赤枠部分)を削ってください。
